 ...
 * @Descripttion:
 * @Author: xiangjun02
 * @Date: 2022-03-06 20:45:54
 * @LastEditors: xiangjun02
 * @LastEditTime: 2022-03-06 21:29:33
 */
import utils from "./utils";
import { join } from "path";
import { statSync, readdirSync, readFileSync, unlinkSync, existsSync, writeFileSync, } from "fs";
import { ctx } from "./Context";
import { window, EventEmitter, Uri, languages } from "vscode";
...
class SnippetNodeProvider {
  ...
  async search() {
    ...
    if (!this.caches[languageId])
      return window.showErrorMessage(`no snippet in language: ${languageId}`);
    let list = this.caches[languageId].list;
    let item = await window.showQuickPick(list, { placeHolder: "" });
    let key = item.label;
  ...
  getChildren(element) {
    if (!element) {
      let filenames = readdirSync(utils.vsCodeSnippetsPath);
      return filenames
  ...
  async deleteGroup(item) {
    let flag = await window.showQuickPick(["No", "Yes"], {
  ...
  async deleteSnippet(e) {
    ...
    cache.t = +new Date();
    writeFileSync(filename, JSON.stringify(cache.data, null, 2), "utf8");
    this.refresh();
  }
  /**
  ...
  async editSnippet(e, def) {
    ...
    let filename = join(
      os.tmpdir(),
      Buffer.from(e.key).toString("base64").replace(/\//g, "-") +
        "." +
        e.languageId +
        ".snippet"
    );
    let content;
    if (!existsSync(filename)) writeFileSync(filename, (content = text));
    let editor = await window.showTextDocument(Uri.file(filename));
    await languages.setTextDocumentLanguage(editor.document, e.languageId);
  ...
  saveSnippet(languageId, key, text) {
    ...
    if (!snippet.prefix) return window.showErrorMessage("@prefix is required");
    if (!snippet.body || !snippet.body.length)
      return window.showErrorMessage("snippet body can't be empty");
    cache.data[key] = snippet;
    cache.t = +new Date();
    writeFileSync(filename, JSON.stringify(cache.data, null, 2), "utf8");
    window.showInformationMessage(
      `snippet "${key}.${languageId}" save success`
    );
    this.tree.reveal({ languageId, label: key });
  }
}
export default SnippetNodeProvider;
